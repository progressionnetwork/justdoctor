...
from justatom.ir.document_store.base import Document
from justatom.ir.document_store.base import BaseDocStore
from justatom.ir.document_store import util
...
class WeaviateDocumentStore(BaseDocStore):
	"""
	...
	def get_document_by_id(self, id: str, index: Optional[str] = None) -> Optional[Document]:
		...
		formatted_id = self._sanitize_id(unique_id=id, index=index)

		result = self.weaviate_client.data_object.get_by_id(formatted_id, with_vector=True)
		if result:
	...
	def get_documents_by_id(self, ids: List[str], index: Optional[str] = None,
		...
		for id in ids:
			id = self._sanitize_id(unique_id=id, index=index)
	...
	def _sanitize_id(self, unique_id: str, index: Optional[str] = None) -> str:
		"""
		...
		"""
		index = self._sanitize_index_name(index) or self.index
		formatted_id = str(uuid.uuid5(uuid.NAMESPACE_URL, unique_id))
		# if not UUID_PATTERN.match(unique_id):
		# 	hashed_id = hashlib.sha256((unique_id+index).encode('utf-8')) #type: ignore
		...
		# 	formatted_id = generated_uuid
		# logger.info(f"Formatted id is {formatted_id}")
		# assert UUID_PATTERN.match(formatted_id)
	...
	def write_documents(
		...
		for do in document_objects:
			do.id = self._sanitize_id(unique_id=do.id, index=index)
		...
		batched_documents = util.get_batches_from_generator(document_objects, batch_size)
		drop_fields = ["score", "probability", "question"]
		with tqdm(total=len(document_objects), disable=not self.progress_bar) as progress_bar:
			for document_batch in batched_documents:
				...
				for idx, doc in enumerate(document_batch):
					...
					_doc[self.content_field] = json.dumps(_doc.pop(self.content_field))
	...
	def update_embeddings(
		...
		for result_batch in util.get_batches_from_generator(result, batch_size):
	...
	def delete_documents(self, index: Optional[str] = None, ids: Optional[List[str]] = None, filters: Optional[Dict[str, List[str]]] = None):
		...
		if not filters and not ids:
			...
			# self._create_schema_and_index_if_not_exist(index) TODO: Do we really need to recreate it?
		else:
