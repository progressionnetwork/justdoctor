...
class Document:
    def __init__(
        ...
        # ALWAYS override id doesn't matter what is provided!
        self.id = self._get_id(id_type)

    def _get_id(self, id_type: str):
        if id_type == "uuid3":
            return str(uuid.uuid3(uuid.NAMESPACE_DNS, self.content))
        elif id_type == "uuid5":
            return str(uuid.uuid5(uuid.NAMESPACE_URL, self.content))
        elif id_type == "mmh3":
            return "{:02x}".format(mmh3.hash128(self.content, signed=False))
        else:
            raise ValueError("Unsupported auto-id generation for type {id_type}. Either {\"uuid\"} or {\"mmh3\"}")
